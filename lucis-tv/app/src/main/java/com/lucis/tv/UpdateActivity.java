...
public class UpdateActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){
        ...
        status=new TextView(this);status.setTextColor(Color.LTGRAY);status.setTextSize(18);status.setText("当前版本："+CURRENT_VERSION+"\n\n直播平台目录会自动更新；这里用于程序版本升级。");root.addView(status);
        action=new Button(this);action.setText("检查程序更新");action.setTextSize(18);action.setFocusable(true);root.addView(action);
    ...
    private void check(){
        ...
        new Thread(()->{
            try{
                HttpURLConnection c=(HttpURLConnection)new URL(META+"?t="+System.currentTimeMillis()).openConnection();c.setConnectTimeout(7000);c.setReadTimeout(7000);c.setUseCaches(false);
                ...
                runOnUiThread(()->{
                    if(remoteCode>CURRENT_CODE&&!apkUrl.isEmpty()){
                        status.setText("发现新版本 "+version+"\n\n"+notes+"\n\n可直接在电视下载 APK 并打开系统安装器。");action.setText("下载并安装更新");action.setEnabled(true);
                    }else{status.setText("当前版本："+CURRENT_VERSION+"\n\n已经是最新程序版本。\n直播平台目录仍会独立在线刷新。");action.setText("重新检查");action.setEnabled(true);}
                });
    ...
    private void download(){
        if(Build.VERSION.SDK_INT>=26&&!getPackageManager().canRequestPackageInstalls()){
            Intent s=new Intent(Settings.ACTION_MANAGE_UNKNOWN_APP_SOURCES,Uri.parse("package:"+getPackageName()));startActivity(s);Toast.makeText(this,"请允许 Lucis TV 安装未知应用，然后返回再点一次更新",Toast.LENGTH_LONG).show();return;
        }
        try{
            DownloadManager dm=(DownloadManager)getSystemService(DOWNLOAD_SERVICE);DownloadManager.Request req=new DownloadManager.Request(Uri.parse(apkUrl));req.setTitle("Lucis TV 更新");req.setDescription("下载完成后打开系统安装界面");req.setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED);req.setMimeType("application/vnd.android.package-archive");
            downloadId=dm.enqueue(req);status.setText("正在下载程序更新…\n\n下载完成后会打开系统安装界面。");action.setEnabled(false);
    ...
    private void installDownloaded(){
        try{
            DownloadManager dm=(DownloadManager)getSystemService(DOWNLOAD_SERVICE);Uri uri=dm.getUriForDownloadedFile(downloadId);if(uri==null)throw new IOException("下载文件不存在");
            Intent in=new Intent(Intent.ACTION_VIEW);in.setDataAndType(uri,"application/vnd.android.package-archive");in.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION|Intent.FLAG_ACTIVITY_NEW_TASK);startActivity(in);
        }catch(Exception e){status.setText("安装器打开失败："+e.getMessage()+"\n\n可在电视下载目录手动打开 APK。");action.setEnabled(true);}
    }
