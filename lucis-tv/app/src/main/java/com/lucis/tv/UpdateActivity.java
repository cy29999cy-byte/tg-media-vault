...
public class UpdateActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){
        ...
        status=new TextView(this); status.setTextColor(Color.LTGRAY); status.setTextSize(18); status.setText("当前版本："+BuildConfig.VERSION_NAME+"\n\n正在检查更新…"); root.addView(status);
        ...
        action.setOnClickListener(v->{ if(remoteCode>BuildConfig.VERSION_CODE&&!apkUrl.isEmpty()) download(); else check(); });
        setContentView(root); check();
    }

    private void check(){
        action.setEnabled(false); status.setText("当前版本："+BuildConfig.VERSION_NAME+"\n\n正在检查更新…");
        new Thread(()->{
            try{
                ...
                runOnUiThread(()->{
                    if(remoteCode>BuildConfig.VERSION_CODE&&!apkUrl.isEmpty()){
                        status.setText("发现新版本 "+version+"\n\n"+notes+"\n\n按下方按钮即可直接在电视上下载更新，不需要再用 U 盘。");
                        action.setText("下载并安装更新"); action.setEnabled(true);
                    }else{
                        status.setText("当前版本："+BuildConfig.VERSION_NAME+"\n\n已经是最新版本。"); action.setText("重新检查"); action.setEnabled(true);
                    }
                });
    ...
    private void download(){
        if(Build.VERSION.SDK_INT>=26 && !getPackageManager().canRequestPackageInstalls()){
            Intent s=new Intent(Settings.ACTION_MANAGE_UNKNOWN_APP_SOURCES, Uri.parse("package:"+getPackageName())); startActivity(s);
            Toast.makeText(this,"请先允许 Lucis TV 安装未知应用，然后返回再点一次更新",Toast.LENGTH_LONG).show(); return;
        }
        try{
            DownloadManager dm=(DownloadManager)getSystemService(DOWNLOAD_SERVICE);
            DownloadManager.Request req=new DownloadManager.Request(Uri.parse(apkUrl)); req.setTitle("Lucis TV 更新"); req.setDescription("下载完成后自动打开安装界面"); req.setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED); req.setMimeType("application/vnd.android.package-archive");
