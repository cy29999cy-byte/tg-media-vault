...
public class MainActivity extends Activity {
    ...
    private void buildUi(){
        seedPublicChannels();
        LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.HORIZONTAL);root.setBackgroundColor(Color.rgb(6,7,11));root.setPadding(30,22,30,22);

        LinearLayout left=new LinearLayout(this);left.setOrientation(LinearLayout.VERTICAL);
        LinearLayout.LayoutParams leftLp=new LinearLayout.LayoutParams(0,-1,4f);leftLp.setMargins(0,0,24,0);root.addView(left,leftLp);

        TextView title=text("LUCIS TV",34);title.setTypeface(null,1);left.addView(title);
        TextView subtitle=text("EA55 · 2.0 · 港片 / 粤语 / 直播平台中心",15);subtitle.setTextColor(Color.LTGRAY);left.addView(subtitle);
        status=text("经典港片直播、虎牙/斗鱼官方影视入口、本地港片库与公开电视直播已整合。",14);status.setTextColor(Color.rgb(184,184,194));left.addView(status);

        Button hkLive=button("🔥 经典港片直播");
        Button platforms=button("🎥 虎牙 / 斗鱼直播平台");
        ...
        left.addView(hkLive);left.addView(platforms);left.addView(resume);left.addView(classics);left.addView(publicLive);left.addView(fav);left.addView(history);left.addView(all);left.addView(importM3u);left.addView(openVideo);left.addView(update);

        TextView note=text("直播平台目录会从云端自动刷新，平台房间和节目内容以虎牙/斗鱼等官方页面当前实际播出为准。",12);note.setTextColor(Color.rgb(135,139,150));left.addView(note);
        ...
        hkLive.setOnClickListener(v->openPortal("港片"));
        platforms.setOnClickListener(v->openPortal("all"));
    ...
    private void openPortal(String filter){Intent i=new Intent(this,PortalActivity.class);i.putExtra("filter",filter);startActivity(i);}
    ...
    private void loadPlaylist(String url){
        ...
        new Thread(()->{
            ...
            try{
                HttpURLConnection c=(HttpURLConnection)new URL(url).openConnection();c.setConnectTimeout(8000);c.setReadTimeout(12000);c.setRequestProperty("User-Agent","LucisTV/2.0");
                ...
                while((line=r.readLine())!=null){
                    line=line.trim();
                    if(line.startsWith("#EXTINF")){int p=line.lastIndexOf(',');if(p>=0&&p+1<line.length())name=line.substring(p+1).trim();String g=attr(line,"group-title");if(!g.isEmpty())group=g;}
                    else if(!line.isEmpty()&&!line.startsWith("#")){loaded.add(new Channel(name,line,group,false));name="频道";group="未分类";}
            ...
            }catch(Exception ex){runOnUiThread(()->status.setText("载入失败："+ex.getMessage()));}
        }).start();
    }
    ...
    private void render(){
        ...
        if(visible.isEmpty()){TextView t=text("暂无可显示内容。",16);t.setTextColor(Color.GRAY);list.addView(t);return;}
        for(Channel c:visible){
            ...
            Button star=button(favs.contains(c.url)?"★":"☆");star.setOnClickListener(v->toggleFavorite(c));row.addView(star,new LinearLayout.LayoutParams(90,-2));list.addView(row);
        }
        if(list.getChildCount()>0){View row=list.getChildAt(0);if(row instanceof ViewGroup&&((ViewGroup)row).getChildCount()>0)((ViewGroup)row).getChildAt(0).requestFocus();}
    }
